...
context_tokens = [[w.text for w in doc] for doc in nlp.pipe(
    context_iter, batch_size=args.batch_size)]
...
train['answer_start_token'], train['answer_end_token'] = \
    zip(*[get_answer_index(a, b, c, d) for a, b, c, d in
          zip(train.context, context_tokens, train.answer_start, train.answer_end)])
...
question_docs = [doc for doc in nlp.pipe(iter(question_text), batch_size=args.batch_size)]
context_docs = [doc for doc in nlp.pipe(iter(context_text), batch_size=args.batch_size)]
if args.wv_cased:
